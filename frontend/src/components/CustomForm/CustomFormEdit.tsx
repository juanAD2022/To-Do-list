...
export const CustomFormEdit=()=>{
    const {value}=useGlobalContextTask()
    ...
    const {control,handleSubmit,formState:{errors}}=useForm<FormValues>(
        {
            ...
            defaultValues:{
                ...
                check_task:value.check_task
            }
        }
    )

    const onSubimit:SubmitHandler<FormValues>=async(data)=>{
        console.log("FORMULARIO EDIT",data)
        const controller = new AbortController();
        try {
            const response = await fetch(`http://127.0.0.1:8000/api/task/${value.id}/`, {
                method:"PUT",
                headers: {
